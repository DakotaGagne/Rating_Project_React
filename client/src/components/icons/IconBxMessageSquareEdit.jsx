// Got from https://reactsvgicons.com/ | React SVG Icons

// icon:bx-message-square-edit | Boxicons https://boxicons.com/ | Atisa
import React from "react";

function IconBxMessageSquareEdit(props) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      height="1em"
      width="1em"
      {...props}
    >
      <path d="M16 2H8C4.691 2 2 4.691 2 8v13a1 1 0 001 1h13c3.309 0 6-2.691 6-6V8c0-3.309-2.691-6-6-6zm4 14c0 2.206-1.794 4-4 4H4V8c0-2.206 1.794-4 4-4h8c2.206 0 4 1.794 4 4v8z" />
      <path d="M7 14.987v1.999h1.999l5.529-5.522-1.998-1.998zm8.47-4.465l-1.998-2L14.995 7l2 1.999z" />
    </svg>
  );
}

export default IconBxMessageSquareEdit;
